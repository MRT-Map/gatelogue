...
def report(node: gt.Node, prefix: str | None = None, ignore: Container[type[gt.Node]] = None):
    ...
    if isinstance(node, gt.AirAirline):
        ...
        if (airports == 0 and gt.AirAirport not in ignore) or (gates == 0 and gt.AirGate not in ignore) or (flights == 0 and gt.AirFlight not in ignore):
            colour = ERROR
        ...
        if (lines == 0 and gt.BusLine not in ignore) or (stops == 0 and gt.BusStop not in ignore) or (berths == 0 and gt.BusBerth not in ignore):
            colour = ERROR
        ...
        if (lines == 0 and gt.SeaLine not in ignore) or (stops == 0 and gt.SeaStop not in ignore) or (docks == 0 and gt.SeaDock not in ignore):
            colour = ERROR
        ...
        if (lines == 0 and gt.RailLine not in ignore) or (stations == 0 and gt.RailStation not in ignore) or (platforms == 0 and gt.RailPlatform not in ignore):
            colour = ERROR
