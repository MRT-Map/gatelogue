<script setup lang="ts">
  ...
  const sortedObjects = computed(() => {
    ...
    return Object.entries(objects[selPanel.value.cat] as any).sort(([_, a], [__, b]) => (a as any)[selPanel.value.objDisplay].localeCompare((b as any)[selPanel.value.objDisplay]))
  })
</script>
...
<template>
  <nav>
    <template v-for="p in panels" :key="p.cat">
      <div class="button" @click="selPanel = p" :class="selPanel.cat === p.cat ? 'sel' : ''"><button><b>{{p.catDisplay}}</b></button></div>
    </template>
    <hr>
    <template v-if="selPanel !== undefined" v-for="[id, o] in sortedObjects" :key="id">
      <RouterLink :to="`/${selPanel.cat}/${id}`">
        <div class="button" :class="sel.cat === selPanel.cat && sel.id === id ? 'sel' : ''">
