...
export class AirAirline extends Node {
  ...
  get flights(): AirFlight[] {
    return this.gd.execGetMany<[number]>("SELECT i FROM AirFlight WHERE airline = ?", [this.i]).map(([i]) => new AirFlight(i, this.gd))
  }
  get gates(): AirGate[] {
    return this.gd.execGetMany<[number]>(`SELECT i FROM AirGate WHERE airline = ? 
      UNION SELECT "from" AS i FROM AirFlight WHERE airline = ? 
      UNION SELECT "to" AS i FROM AirFlight WHERE airline = ?`, [this.i]).map(([i]) => new AirGate(i, this.gd))
  }
  get airports(): AirAirport[] {
    return this.gd.execGetMany<[number]>(`SELECT DISTINCT airport FROM AirGate WHERE airline = ? 
      UNION SELECT DISTINCT airport FROM AirFlight LEFT JOIN AirGate on AirGate.i = "from" WHERE AirFlight.airline = ? 
      UNION SELECT DISTINCT airport FROM AirFlight LEFT JOIN AirGate on AirGate.i = "to" WHERE AirFlight.airline = ?`).map(([i]) => new AirAirport(i, this.gd))
  }
}
...
export class AirAirport extends LocatedNode {
  ...
  get gates(): AirGate[] {
    return this.gd.execGetMany<[number]>("SELECT i FROM AirGate WHERE airport = ?", [this.i]).map(([i]) => new AirGate(i, this.gd))
  }
}
...
export class AirGate extends Node {
  ...
  get flightsFromHere(): AirFlight[] {
    return this.gd.execGetMany<[number]>("SELECT i FROM AirFlight WHERE \"from\" = ?", [this.i]).map(([i]) => new AirFlight(i, this.gd))
  }

  get flightsToHere(): AirFlight[] {
    return this.gd.execGetMany<[number]>("SELECT i FROM AirFlight WHERE \"to\" = ?", [this.i]).map(([i]) => new AirFlight(i, this.gd))
  }
}
