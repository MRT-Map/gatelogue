...
import initSqlJs, {type Database, type SqlJsStatic, type BindParams, type SqlValue} from "sql.js";
...
import {BusBerth, BusCompany, BusConnection, BusLine, BusStop} from "./bus.js";
import {RailCompany, RailConnection, RailLine, RailPlatform, RailStation} from "./rail.js";
import {SeaCompany, SeaConnection, SeaDock, SeaLine, SeaStop} from "./sea.js";
import {SpawnWarp} from "./spawnWarp.js";
import {Town} from "./town.js";

const SQL: SqlJsStatic = typeof window === 'undefined' ? await initSqlJs() : await initSqlJs({locateFile: file => `https://sql.js.org/dist/${file}`});

// TODO
const URL = "https://raw.githubusercontent.com/MRT-Map/gatelogue/refs/heads/dist-v3/data.db";
const URL_NO_SOURCES = "https://raw.githubusercontent.com/MRT-Map/gatelogue/refs/heads/dist-v3/data-ns.db";
...
export class GD {
  ...
  execGetMany<T extends SqlValue[]>(sql: string, params?: BindParams): T[] {
    return this.db.exec(sql, params)[0]!.values as T[]
  }
  ...
  get hasSources(): boolean {
    return this.execGetOne<[number]>("SELECT has_sources FROM Metadata")[0] === 1
  }

  // eslint-disable-next-line complexity
  getNode(i: ID): Node {
    const ty = this.execGetOne<[string]>("SELECT type FROM Node WHERE i = ?", [i])[0]!;
    switch (ty) {
      case "AirAirline": return new AirAirline(i, this)
      case "AirAirport": return new AirAirport(i, this)
      case "AirGate": return new AirGate(i, this)
      case "AirFlight": return new AirFlight(i, this)
      case "BusCompany": return new BusCompany(i, this)
      case "BusLine": return new BusLine(i, this)
      case "BusStop": return new BusStop(i, this)
      case "BusBerth": return new BusBerth(i, this)
      case "BusConnection": return new BusConnection(i, this)
      case "SeaCompany": return new SeaCompany(i, this)
      case "SeaLine": return new SeaLine(i, this)
      case "SeaStop": return new SeaStop(i, this)
      case "SeaDock": return new SeaDock(i, this)
      case "SeaConnection": return new SeaConnection(i, this)
      case "RailCompany": return new RailCompany(i, this)
      case "RailLine": return new RailLine(i, this)
      case "RailStation": return new RailStation(i, this)
      case "RailPlatform": return new RailPlatform(i, this)
      case "RailConnection": return new RailConnection(i, this)
      case "SpawnWarp": return new SpawnWarp(i, this)
      case "Town": return new Town(i, this)
      default: throw new Error(`Unknown type ${ty}`)
    }
  }
  get nodes(): Node[] {
    return this.execGetMany<[number]>("SELECT i FROM Node").map(([i]) => Node.fromId(i, this))
  }
  get locatedNodes(): LocatedNode[] {
    return this.execGetMany<[number]>("SELECT i FROM NodeLocation").map(([i]) => LocatedNode.fromId(i, this))
  }
  protected nodesByType(ty: string): ID[] {
    return this.execGetMany<[number]>("SELECT i FROM Node WHERE type = ?", [ty]).map(([i]) => i)
  }
  ...
  get busConnections(): BusConnection[] {
    return this.nodesByType("BusConnection").map(i => new BusConnection(i, this))
  }
  ...
  get RailPlatforms(): RailPlatform[] {
    return this.nodesByType("RailPlatform").map(i => new RailPlatform(i, this))
  }
  get railConnections(): RailConnection[] {
    return this.nodesByType("RailConnection").map(i => new RailConnection(i, this))
  }
  ...
  get seaConnections(): SeaConnection[] {
    return this.nodesByType("SeaConnection").map(i => new SeaConnection(i, this))
  }
